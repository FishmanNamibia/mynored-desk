...
export async function GET() {
  try {
    const vision: any = await prisma.$queryRaw`
      SELECT * FROM "Vision" WHERE id = 'vision-singleton' LIMIT 1
    `;
    return NextResponse.json(vision[0] || null);
  } catch (error) {
...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (file) {
      ...
      const fileName = `vision-${timestamp}-${sanitizedFileName}`;
